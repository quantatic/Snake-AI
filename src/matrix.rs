...
impl<T> Matrix<T> {
    pub fn new_map<F>(height: usize, width: usize, mut func: F) -> Self where
        F: FnMut(usize, usize) -> T,
    {
        ...
        for row in 0..height {
            for col in 0..width {
                options_matrix[row][col] = Some(
                    func(row, col)
                )
            }
        }

        let result_values = options_matrix.values
            .into_iter()
            .map(|option_value: Option<T>| {
                option_value.unwrap()
            })
            .collect::<Vec<T>>();
...
impl<T> Index<usize> for Matrix<T> {
    ...
    fn index(&self, row: usize) -> &Self::Output{
        if row >= self.height {
            panic!(
                "Indexing with row {} is invalid for matrix with height of {}", row, self.height
            );
        }
    ...
    T: Add<T, Output = T>,
{
    ...
    fn add(self, rhs: Self) -> Self::Output {        
        if self.width != rhs.width {
            panic!(
                "lhs width is {} and rhs width is {}, which is an invalid combination", self.width, rhs.width
            );
        }

        if self.height != rhs.height {
            panic!(
                "lhs height is {} and rhs height is {}, which is an invalid combination", self.height, rhs.height
            );
        }

        let values = self.values.into_iter()
            .zip(rhs.values.into_iter())
            .map(|(lhs_value, rhs_value): (T, T)| {
                lhs_value + rhs_value
            })
            .collect::<Vec<T>>();
    ...
    T: AddAssign<T>,
{
    fn add_assign(&mut self, rhs: Self) {
        ...
        self.values.iter_mut()
            .zip(rhs.values.into_iter())
            .for_each(|(lhs_mut_ref, rhs_value): (&mut T, T)| {
                *lhs_mut_ref += rhs_value
            });
    }
}
...
impl<'a, T> Sub<&'a Matrix<T>> for &'a Matrix<T> where
    ...
    T: Mul<T, Output = T> + Add<T, Output = T> + Copy
{
    ...
    fn mul(self, rhs: Self) -> Self::Output {
        if self.width != rhs.height {
            panic!(
                "lhs width is {} and rhs height is {}, which is an invalid combination", self.width, rhs.height
            );
        }

        Matrix::new_map(self.height, rhs.width, |row, col| {
            let mut values_iter = (0..self.width)
                .map(|i| {
    ...
    T: Mul<T, Output = T> + Copy
{
    ...
    fn mul(self, rhs: T) -> Self::Output {
        Matrix::new_map(self.height, self.width, |row, col| {
            self[row][col] * rhs
        })
    }
}
