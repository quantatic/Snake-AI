...
impl Network {
    ...
    pub fn new(shape: Vec<usize>) -> Self {
        ...
        let weights = shape.iter()
            .zip(shape[1..].iter())
            .map(|(&input_size, &output_size): (&usize, &usize)| {
                Matrix::new_map(
                    input_size, output_size, |_row: usize, _col: usize| {
    ...
    pub fn evaluate(&self, values: Vec<f64>) -> Vec<f64> {
        let input: Matrix<f64> = Matrix::new_map(
            1, values.len(), |_row: usize, col: usize| {
                values[col]
            }
        );
    ...
    pub fn render(&self, canvas: &mut Canvas<Window>, scale: u16) {
        ...
        for (start_layer, (&start_layer_size, &end_layer_size)) in self.shape.iter()
            .zip(self.shape[1..].iter())
            .enumerate() {
                for start_node in 0..start_layer_size {
                    let start_point = Point::new(
                        ((start_layer as i32) * i32::from(scale) * space_between) + i32::from(scale / 2) + x_offset,
                        ((start_node as i32) * i32::from(scale) * space_between) + i32::from(scale / 2) + y_offset
                    );
                    for end_node in 0..end_layer_size {
                        let end_point = Point::new(
                            (((start_layer + 1) as i32) * i32::from(scale) * space_between) + i32::from(scale / 2) + x_offset,
                            ((end_node as i32) * i32::from(scale) * space_between) + i32::from(scale / 2) + y_offset
                        );
