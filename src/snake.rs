...
impl Game {
    ...
    pub fn new(width: u16, height: u16, tile_size: u16) -> Self {
        ...
        snake.push(Location { x: 0, y: 0 });

        snake.push(Location { x: 1, y: 0 });

        snake.push(Location { x: 2, y: 0 });
    ...
    pub fn render(&mut self, canvas: &mut Canvas<Window>) {
        ...
        for y in 0..self.height {
            for x in 0..self.width {
                ...
                let snake_here = self.snake.iter().any(|&snake_loc| snake_loc == curr_loc);
    ...
    pub fn step(&mut self) -> GameStatus {
        ...
        let collision_with_self = self.snake.iter().any(|&snake_loc| snake_loc == new_front);
        ...
        if new_front == self.food_loc {
            ...
            while food_overlap {
                ...
                food_overlap = self
                    .snake
                    .iter()
                    .any(|&snake_loc| snake_loc == self.food_loc);
            }
        ...
        for snake_location in self.snake.iter().rev().skip(1).rev() {
            ...
            if snake_bit_difference_x == 0 {
                if snake_bit_difference_y >= 0 {
                    distance_to_obstacle_down =
                        i16::min(distance_to_obstacle_down, snake_bit_difference_y);
                }

                if snake_bit_difference_y <= 0 {
                    distance_to_obstacle_up =
                        i16::min(distance_to_obstacle_up, -snake_bit_difference_y);
                }
            }

            if snake_bit_difference_y == 0 {
                if snake_bit_difference_x >= 0 {
                    distance_to_obstacle_left =
                        i16::min(distance_to_obstacle_left, snake_bit_difference_x);
                }

                if snake_bit_difference_x <= 0 {
                    distance_to_obstacle_right =
                        i16::min(distance_to_obstacle_right, -snake_bit_difference_x);
                }
            }
        }

        GameStatus::InProgress(GameStats {
